...
experiment = args[3]
given_input_replicate = args[4]

other_input_replicates = setdiff(grep("IN_[0-9]+$", names(count_data), value=TRUE), given_input_replicate)

print(paste0("output/figures/input_distributions/", experiment, ".", given_input_replicate, ".", other_input_replicates[1], '.input_distribution.pdf'))

count_data$pct_gc = nuc_data$`8_pct_gc`

count_gc_data = count_data[select(count_data, matches("(IP|IN)_[0-9]*$")) %>% rowSums > 0,] %>% group_by(gc_bin = cut_number(pct_gc,10))
