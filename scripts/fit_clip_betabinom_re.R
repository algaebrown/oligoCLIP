...
distribution_dir = file.path(dirname(coef_dir), "figures", "clip_scatter_re")
dir.create(coef_dir, showWarnings = FALSE, recursive = TRUE) # "output/figures/clip_scatter_re/" or "output/figures/clip_scatter_re/"
dir.create(distribution_dir, showWarnings = FALSE, recursive = TRUE) # "output/figures/clip_scatter_re/"

sample_cols = names(re_data)[2:ncol(re_data)] # re data has only 1 column stating the type of repeat
...
clip_betabinom_re_fit_data = lapply(other_clip_replicates, function(other_clip_replicate)
    {
        ...
        pdf(paste0(distribution_dir, experiment, ".", given_clip_replicate, ".", other_clip_replicate, '.clip_null_distribution.pdf'),height = 1.8, width = 2.8)
        print(
...
write_tsv(clip_betabinom_re_fit_data, outf)