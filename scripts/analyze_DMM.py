...
if __name__=='__main__':
    ...
    annotation_summary = pd.concat([anno.sum(axis = 1), ent], axis = 1)
    annotation_summary.columns = ['# RBP assigned', 'RI to total read distribution']
    ...
    # jaccard index
    d_dmm = compute_jaccard_index(individual_bfs_dmm.ge(logLR_threshold))
    cm=sns.clustermap(d_dmm, cmap = 'Greys', metric = 'correlation', figsize = (4,4),
                vmax = 0.3)
    cm.cax.set_visible(False)
    plt.suptitle('ABC(Dirichlet Mixture Model: individual BF)', y = 1)

    indiv_assignment_fcount = count_by_rbp(data_bf_dmm, anno)
    ...
    for c in anno.columns:
        ...
        enriched_windows = data_bf_dmm.loc[data_bf_dmm[c], columns]
    ...
    megaoutput = pd.concat([data_bf_dmm, p_bar, p_raw, fc_raw, fc_bar, std_bar, individual_bfs_dmm], axis = 1)
