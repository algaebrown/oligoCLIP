...
# count_gc_data = count_data[count_data[c(clip_replicate_label, input_replicate_label)] %>% rowSums > 0,] 
#%>% group_by(gc_bin = cut_number(pct_gc,n_bin)) # using only labels of use
n_bins = 20
selected_re_data = re_data %>% rename(input = all_of(input_replicate_label), clip = all_of(clip_replicate_label)) %>% 
	...
	mutate(gc_bin = cut_number(gc,n_bins)) %>% group_by(gc_bin) %>%
...
pdf(file.path(scatter_re_path,paste0(output_stem, '.clip_test_distribution.pdf')),height = 1.8, width = 2.8)
...
write_tsv(q_data %>% select(-above_threshold), paste0(enrich_dir, output_stem, ".enriched_re.tsv.gz"))