...
if __name__=='__main__':
    ...
    variants = pd.read_csv(sys.argv[1],
        sep = '\t',
        names = ['CHROM','POS','ID','REF', 'ALT', 'INFO/AC', 'INFO/AN']
    )
    ...
    if variants.empty:
        ...
        df = BedTool.from_dataframe(variants[['CHROM','POS-1','POS','ID','REF', 'ALT','INFO/AC', 'INFO/AN']]).intersect(
            window_bed, wb = True).to_dataframe(names = ['CHROM','POS-1', 'POS','ID','REF', 'ALT','INFO/AC', 'INFO/AN']+window_df.columns.tolist())
