...
def test_align_reads_to_Drosophila():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(".tests/unit/align_reads_to_Drosophila/data")
        expected_path = PurePosixPath(".tests/unit/align_reads_to_Drosophila/expected")

        # Copy data to the temporary workdir.
        shutil.copytree(data_path, workdir)

        # dbg
        print("K562_rep1/bams/dros/IGF2BP2.Aligned.out.bam K562_rep1/bams/dros/IGF2BP2.Unmapped.out.mate1 K562_rep1/bams/dros/IGF2BP2.Log.final.out", file=sys.stderr)
