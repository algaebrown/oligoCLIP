...
rule quantify_repeats:
    ...
    output:
        counts = "output/counts/repeats/vectors/{libname}.{sample_label}.counts"
    params:
        error_out_file = "stderr/{libname}.{sample_label}.quantify_repeats.err",
        out_file = "stdout/{libname}.{sample_label}.quantify_repeats.out",
        run_time = "15:00",
        memory = "20000",
        job_name = "dedup_bam",
...
rule make_repeat_count_tables:
    input:
        ...
        replicate_counts = lambda wildcards: expand(
            "output/counts/repeats/vectors/{libname}.{sample_label}.counts", 
            libname = experiment_to_libname(wildcards.experiment), # TODO: make dictionary
            sample_label = [wildcards.sample_label]),
    output:
        name_table = "output/counts/repeats/tables/name/{experiment}.{sample_label}.tsv.gz",
        class_table = "output/counts/repeats/tables/class/{experiment}.{sample_label}.tsv.gz",
        family_table = "output/counts/repeats/tables/family/{experiment}.{sample_label}.tsv.gz",
    params:
        error_out_file = "stderr/{experiment}.{sample_label}.make_repeat_count_tables.err",
        out_file = "stdout/{experiment}.{sample_label}.make_repeat_count_tables.out",
        run_time = "00:15:00",
        cores = "1",
        memory = "200",
...
rule fit_clip_betabinomial_re_model:
    input:
        table = lambda wildcards: "output/counts/repeats/tables/name/"+libname_to_experiment(wildcards.libname)+".{sample_label}.tsv.gz",
    output:
        coef = "output/clip_model_coef_re/{libname}.{sample_label}.tsv",
    ...
    params:
        error_out_file = "error_files/{libname}.{sample_label}.fit_clip_betabinomial_re_model.err",
        out_file = "stdout/{libname}.{sample_label}.fit_clip_betabinomial_re_model.out",
        run_time = "10:00",
        memory = "2000",
...
rule sum_all_other_background_re:
    input:
        lambda wildcards: expand("output/counts/repeats/vectors/{libname}.{sample_label}.counts",
        libname = ["{libname}"],
        sample_label = list(set(rbps)-set([wildcards.sample_label])-set([config['AS_INPUT']]))
        )
    output:
        counts= "internal_output/counts/repeats/vectors/{libname}.{sample_label}.counts",
    params:
        error_out_file = "error_files/{libname}.{sample_label}.sum_read.err",
        out_file = "stdout/{libname}.{sample_label}.sum_read.out",
        run_time = "20:00",
        cores = "1",
        memory = "10000",
...
rule combine_ip_to_internal_background: # bg_sample_label = 'internal'
    input:
        count_table = "output/counts/repeats/tables/name/{experiment}.{sample_label}.tsv.gz",
        bg_counts = lambda wildcards: expand("internal_output/counts/repeats/vectors/{libname}.{sample_label}.counts", 
            libname = experiment_to_libname(wildcards.experiment), # TODO: make dictionary
            sample_label = [wildcards.sample_label])
    output:
        combined_count_table = "internal_output/counts/repeats/bgtables/internal/{experiment}.{sample_label}.tsv.gz"
    params:
        error_out_file = "error_files/{experiment}.internal.{sample_label}.combine.err",
        out_file = "stdout/{experiment}.internal.{sample_label}.combine.out",
        run_time = "1:00:00",
        cores = "1",
        memory = "10000",
...
rule call_enriched_re:
    input:
        table = lambda wildcards: f"internal_output/counts/repeats/bgtables/internal/"+libname_to_experiment(wildcards.libname)+f".{wildcards.sample_label}.tsv.gz",
        repeats = rules.uniq_repeats.output.unique_repeats,
        parameters = "output/clip_model_coef_re/{libname}.{sample_label}.tsv",
    output:
        "internal_output/figures/clip_scatter_re/{libname}.{sample_label}.clip_test_distribution.pdf",
        "internal_output/enriched_re/{libname}.{sample_label}.enriched_re.tsv.gz"
    params:
